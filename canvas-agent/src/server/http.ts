...
import * as officialCodex from "../agent/codex.js";
import * as lightweightAgent from "../agent/lightweight-agent.js";
import { configureLightweightAgent } from "../agent/lightweight-agent.js";
...
const useLightweightAgent = process.env.AICOST_EMBEDDED_AGENT === "1" || process.env.CANVAS_AGENT_MODE === "lightweight";
const {
    archiveCodexThread,
    CodexSkillLookupError,
    configureCodexSkill,
    generateCodexSkillDraft,
    interruptCodexTurn,
...
} = (useLightweightAgent ? lightweightAgent : officialCodex) as typeof officialCodex;
...
export function startHttpServer() {
    ...
    if (useLightweightAgent) configureLightweightAgent((name, input) => session.callTool(name, input));
    ...
    app.post("/agent/ai-config", (req, res) => {
        ...
        const next = {
            ...
            ...(typeof body.textModel === "string" ? { textModel: body.textModel } : {}),
            ...(Array.isArray(body.textModels) ? { textModels: body.textModels.map(String) } : {}),
            ...(typeof body.systemPrompt === "string" ? { systemPrompt: body.systemPrompt } : {}),
        };
        config.ai = next;
        saveConfig(config);
        res.json({ ok: true, ai: { ...next, apiKey: next.apiKey ? "***" : "" } });
    });
    ...
    app.listen(port, "127.0.0.1", () => {
        console.log(useLightweightAgent ? "佳速画布 Agent" : "Infinite Canvas Agent");
        checkVersions();
