...
export function checkVersions() {
    if (process.env.AICOST_EMBEDDED_AGENT === "1" || process.env.CANVAS_AGENT_MODE === "lightweight") {
        logger.info("Canvas Agent version", { version: VERSION, mode: "lightweight" });
        return;
    }
    const localCodexVersion = commandVersion("codex");
    logger.info("Canvas Agent version", { version: VERSION });
    logger.info("Bundled Codex version", { version: CODEX_VERSION || "not bundled" });
    logger.info("Local Codex version", { version: localCodexVersion || "not found" });
    if (!CODEX_VERSION) {
        logger.warn("Bundled Codex package was not found in this install.");
    } else if (!localCodexVersion) {
