...
export function UserStatusActions({ showConfig = true, variant = "default", onOpenShortcuts, onOpenPlugins }: UserStatusActionsProps) {
    ...
    return (
        <div className="inline-flex shrink-0 items-center gap-1">
            {user ? (
                <>
                    <span className="hidden max-w-[7.5rem] truncate px-1 text-xs font-medium sm:inline" style={iconStyle} title={user.displayName || user.username}>
                        {user.displayName || user.username}
                    </span>
                    <button type="button" className={naturalIconClass} style={iconStyle} onClick={() => logout()} aria-label={t("topNav.logout")} title={t("topNav.logout")}>
