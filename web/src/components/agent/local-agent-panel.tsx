...
import { acknowledgeCodexHistory, activateAgentClient, AgentApiError, discoverAgentConfig, fetchAgentJson, interruptCodexTurn, postAgentAiConfig, postCodexApproval, postState, postToolResult } from "@/services/api/canvas-agent";
...
export function LocalAgentPanel({ embedded, headless, autoConnect }: { embedded?: boolean; headless?: boolean; autoConnect?: boolean }) {
    ...
    const toggleAgentConnection = async ({ silent = false }: { silent?: boolean } = {}) => {
        ...
        let desktopToken = "";
        let desktopError = "";
        const desktop = window.interfaceCanvasDesktop;
        if (!urlToken && desktop?.getCanvasAgentInfo) {
            try {
                const info = desktop.ensureCanvasAgentStarted
                    ? await desktop.ensureCanvasAgentStarted()
                    : await desktop.getCanvasAgentInfo();
                desktopUrl = String(info?.url || "").trim().replace(/\/$/, "");
                desktopToken = String(info?.token || "").trim();
                desktopError = String(info?.error || "").trim();
                if (!desktopToken || info?.started === false) {
                    const text = desktopError || rt("embeddedAgentFailed");
                    setAgentState({ connectError: text, activity: rt("embeddedAgentFailed") });
                    if (!silent && !headless) message.warning(text);
                    return;
                }
            } catch (error) {
                desktopError = error instanceof Error && error.message ? error.message : rt("embeddedAgentFailed");
                setAgentState({ connectError: desktopError, activity: rt("embeddedAgentFailed") });
                if (!silent && !headless) message.warning(desktopError);
                return;
            }
        }
        const discovered = urlToken || desktopToken || desktop ? null : await discoverAgentConfig(desktopUrl || endpoint || DEFAULT_AGENT_URL);
        const nextEndpoint = (urlEndpoint || desktopUrl || discovered?.url || endpoint || DEFAULT_AGENT_URL).trim().replace(/\/$/, "");
        const nextToken = (urlToken || desktopToken || token.trim() || discovered?.token || "").trim();
        if (!nextEndpoint) {
        ...
        if (!nextToken) {
            const text = desktopError || rt(desktop ? "embeddedAgentFailed" : "agentNotFound");
            setAgentState({ connectError: text });
            if (!silent && !headless) message.warning(text);
            return;
        }
    ...
    const desktopAutoConnect = typeof window !== "undefined" && Boolean(window.interfaceCanvasDesktop);
    useEffect(() => {
        if ((!autoConnect && !urlAgentAutoConnect && !desktopAutoConnect) || autoConnectRef.current || enabled || connected) return;
        autoConnectRef.current = true;
        void toggleAgentConnection({ silent: true });
    }, [autoConnect, connected, desktopAutoConnect, enabled, urlAgentAutoConnect]);

    const buildAgentAiConfig = () => {
        const config = useConfigStore.getState().config;
        const channel = resolveModelChannel(config, config.textModel);
        const textModel = modelOptionName(config.textModel);
        const textModels = channel.models.filter((model) => model.capability === "text").map((model) => model.name);
        return {
            baseUrl: channel.baseUrl,
            apiKey: channel.apiKey,
            textModel,
            textModels: textModels.length ? textModels : textModel ? [textModel] : [],
            systemPrompt: config.systemPrompt,
        };
    };

    const syncAgentAiConfig = async (nextEndpoint = endpoint, nextToken = token) => {
        const base = nextEndpoint.trim().replace(/\/$/, "");
        const auth = nextToken.trim();
        if (!base || !auth) return;
        const ai = buildAgentAiConfig();
        try {
            if (window.interfaceCanvasDesktop?.setCanvasAgentAiConfig) {
                await window.interfaceCanvasDesktop.setCanvasAgentAiConfig(ai);
            }
            await postAgentAiConfig(base, auth, ai);
        } catch {
            // Settings sync should not block chat connection.
        }
    };

    useEffect(() => {
        if (!connected) return;
        void syncAgentAiConfig();
    }, [connected, endpoint, token]);
