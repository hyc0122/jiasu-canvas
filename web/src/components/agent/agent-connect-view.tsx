...
export function AgentConnectView({
    ...
    const isDesktop = typeof window !== "undefined" && Boolean(window.interfaceCanvasDesktop);
    ...
    const statusText = connectError
        ? (isDesktop ? t("agent.connect.embeddedFailed") : t("agent.status.failed"))
        : connected
            ? (isDesktop ? t("agent.connect.embeddedConnected") : activity)
            : enabled
                ? (isDesktop ? t("agent.connect.embeddedStarting") : t("agent.status.connecting"))
    ...
    return (
        <div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto p-4">
            <div className="space-y-4">
                <div>
                    <div className="text-base font-semibold leading-6">{t(isDesktop ? "agent.connect.embeddedTitle" : "agent.connect.title")}</div>
                    <div className="mt-1 text-xs leading-5" style={{ color: theme.node.muted }}>
                        {t(isDesktop ? "agent.connect.embeddedDescription" : "agent.connect.description")}
                    </div>
                </div>
                {isDesktop ? null : <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                            <div className="flex min-w-0 items-center gap-2">
                                <span className="shrink-0 text-sm font-medium leading-5">{t(isDesktop ? "agent.connect.embeddedTitle" : "agent.connect.webConnection")}</span>
                                <span
                            ...
                            <div className="mt-1 text-xs leading-5" style={{ color: theme.node.muted }}>
                                {t(isDesktop ? "agent.connect.embeddedAuto" : "agent.connect.autoDiscover")}
                            </div>
                        </div>
