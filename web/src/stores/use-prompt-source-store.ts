    ...
    persist(
        ...
        }),
        {
            ...
            merge: (persisted, current) => {
                ...
                const DAVIDWU_ID = "davidwu-gpt-image2-prompts";
                const builtIn = DEFAULT_PROMPT_SOURCES.map((source) => ({ ...source, enabled: source.id === DAVIDWU_ID }));
                const custom = savedSources.filter((source) => !source.builtIn).map((source) => createPromptSource({ ...source, enabled: false }));
