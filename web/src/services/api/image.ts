...
import { ensureXinyunPublicUrl, requestXinyunImages, streamXinyunChat } from "./xinyun";
...
export async function requestGeneration(config: AiConfig, prompt: string, options?: RequestOptions) {
    ...
    if (requestConfig.apiFormat === "xinyun") {
        try {
            const urls = await requestXinyunImages(requestConfig, withSystemPrompt(requestConfig, prompt), [], n, options);
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], options?: RequestOptions) {
    ...
    if (requestConfig.apiFormat === "xinyun") {
        try {
            const refs = await Promise.all(references.map(async (image) => ensureXinyunPublicUrl(requestConfig, await imageToDataUrl(image), "image", options)));
            const urls = await requestXinyunImages(requestConfig, withSystemPrompt(requestConfig, requestPrompt), refs, n, options);
            return urls.map((dataUrl) => ({ id: nanoid(), dataUrl }));
        } catch (error) {
            throw new Error(readAxiosError(error, apiText("requestFailed")));
        }
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    ...
    try {
        ...
        if (requestConfig.apiFormat === "xinyun") {
            const chatMessages = withSystemMessage(requestConfig, messages)
                .filter((message): message is AiTextMessage => !("type" in message) && message.role !== "tool")
                .map((message) => ({ role: message.role, content: message.content }));
            const answer = (await streamXinyunChat(requestConfig, chatMessages, onDelta, options)).trim() || apiText("noContent");
            if (answer === apiText("noContent")) onDelta(answer);
            return answer;
        }
