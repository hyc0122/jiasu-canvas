...
export type AgentAiConfigPayload = {
    baseUrl?: string;
    apiKey?: string;
    textModel?: string;
    textModels?: string[];
...
export function postAgentAiConfig(endpoint: string, token: string, ai: AgentAiConfigPayload) {
    return fetchAgentJson<{ ok?: boolean }>(endpoint, token, "/agent/ai-config", jsonPost(ai));
}
