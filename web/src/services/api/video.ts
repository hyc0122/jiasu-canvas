...
import { createXinyunVideoTask, ensureXinyunPublicUrl, pollXinyunVideoTask, type XinyunMediaItem } from "./xinyun";
...
export type VideoGenerationTask = { id: string; provider: "openai" | "gemini" | "xinyun" | "plugin"; model: string };
...
export async function createVideoGenerationTask(config: AiConfig, prompt: string, references: ReferenceImage[] = [], options?: VideoMediaOptions): Promise<VideoGenerationTask> {
    ...
    if (requestConfig.apiFormat === "xinyun") return createXinyunProviderVideoTask(requestConfig, selectedModel, prompt, references, options);
...
export async function pollVideoGenerationTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    ...
    if (task.provider === "xinyun") return pollXinyunProviderVideoTask(requestConfig, task, options);
...
async function createXinyunProviderVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: VideoMediaOptions): Promise<VideoGenerationTask> {
    const mode = resolveVideoMode(config.videoMode, references.length);
    const imageUrls = await Promise.all(references.map(async (image) => ensureXinyunPublicUrl(config, await imageToDataUrl(image), "image", options)));
    const videoUrls = await Promise.all((options?.videos || []).map(async (video) => {
        const file = await referenceMediaToFile(video, "ref.mp4", "invalidReferenceVideo", options);
        return ensureXinyunPublicUrl(config, file, "video", options);
    }));
    const audioUrls = await Promise.all((options?.audios || []).map(async (audio) => {
        const file = await referenceMediaToFile(audio, "ref.mp3", "invalidReferenceAudio", options);
        return ensureXinyunPublicUrl(config, file, "audio", options);
    }));

    let images: XinyunMediaItem[] | undefined;
    let audios: XinyunMediaItem[] | undefined;
    let videos: XinyunMediaItem[] | undefined;

    if (mode === "frames") {
        images = [];
        if (imageUrls[0]) images.push({ url: imageUrls[0], type: "first_frame" });
        if (imageUrls[1]) images.push({ url: imageUrls[1], type: "end_frame" });
        // Do not send audios with first_frame/end_frame.
        if (videoUrls.length) videos = videoUrls;
    } else {
        if (imageUrls.length) images = imageUrls;
        if (videoUrls.length) videos = videoUrls;
        if (audioUrls.length) audios = audioUrls;
    }

    const taskId = await createXinyunVideoTask(
        { ...config, model },
        prompt,
        { images, videos, audios },
        options,
    );
    return { id: taskId, provider: "xinyun", model };
}

async function pollXinyunProviderVideoTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    const state = await pollXinyunVideoTask(config, task.id, options);
