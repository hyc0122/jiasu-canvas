...
+ [新增] 桌面热更新（asar）：设置/关于中可检查更新并下载安装重启；manifest 来自 ai-updates 公共桶。
+ [新增] 热更测试可见标记：设置页与版本弹窗显示「热更测试包 0.18.10」。

## v0.18.9 - 2026-09-14

+ [新增] 桌面热更新客户端（检查 / 下载 / 校验 sha256 / 分离 apply 脚本重启）；打包版可用。
+ [新增] `desktop/scripts/publish-update.cjs` 发布 app.asar + latest.json 到 ai-updates。

## v0.18.8 - 2026-09-14

+ [修复] 鉴权等待页不再只显示「心云画布」暗色字样，改为方案 D 分步清单 + 陀螺仪（检查环境 / 恢复会话 / 启动画布服务 / 打开工作台）。
+ [修复] 登录/2FA 成功后用方案 C 雷达启动面板整页替换右侧表单，进度条与百分比约 3 秒后再进入工作台，桌面端顺带 ensure canvas-agent。

## v0.18.7 - 2026-09-14

+ [新增] 桌面冷启动增加心云画布启动清单闪屏（检查环境 / 启动画布服务 / 加载界面 / 打开工作台），Agent 超时约 12 秒不阻塞进主窗口。
+ [新增] 登录成功后展示本地工作台启动进度条（雷达动画），桌面端顺带 ensure canvas-agent。
+ [新增] 登录页支持忘记密码：发送重置邮件、确认令牌、展示生成的新密码并可回填登录；支持 `/user/reset` 深链。

## v0.18.6 - 2026-09-13

+ [调整] 桌面 `session.json` 明文保存密码以便登录框回填；保持 0.18.5 子进程 Agent 启动方式。

## v0.18.5 - 2026-09-13

+ [修复] 桌面端不再把 canvas-agent 导入 Electron 主进程，改为以 `ELECTRON_RUN_AS_NODE` 子进程启动，避免 Windows 上 net.fetch 与同进程 HTTP 死锁导致启动卡死/报错。
+ [修复] `whenReady` 先创建窗口再异步拉起 Agent，界面不再等待 Agent 健康检查。
+ [修复] 鉴权 splash 若 zustand persist 超过 2 秒仍未 hydrated，强制 hydrated 并 bootstrap，避免登录页永远转圈。
+ [优化] 主进程异常写入 userData `main.log`，Agent 子进程 stdout/stderr 写入 `agent.log`。


## v0.18.4 - 2026-09-13

+ [修复] 桌面安装包内置 Agent 不再漏打 `node_modules`，打包前先暂存运行时依赖，启动失败时显示真实错误而不是 Codex/npx 说明。
+ [修复] 登录会话写入 userData `session.json` 并保存吴心 Cookie，重启后保持登录；未点退出则自动恢复，登录框只回填用户名。
+ [修复] 鉴权不再在 zustand persist 水合前把会话标成已检查，避免重启后被踢回登录页。


## v0.18.2 - 2026-09-13

+ [修复] 桌面端注册「获取验证码」改为主进程直接请求吴心接口，并显示成功/失败提示，避免点了没反应、邮件发不出去。

## v0.18.1 - 2026-09-13

+ [修复] 注册页点击「获取验证码」时邮箱为空会报 Cannot read properties of undefined (reading 'trim')。
+ [调整] 产品对外名称改为心云画布；注册邀请码预留但不展示。


+ [新增] 接入吴心账号登录与注册：未登录进入业务页会先到 `/login`，左侧循环背景视频，右侧登录/注册；注册成功后需再登录，开启 2FA 时补验证码。
+ [调整] 产品对外名称改为云心画布。
+ [新增] 增加 Electron 桌面封装，可打包 Windows NSIS/便携版和 Linux AppImage、deb、tar.gz。
