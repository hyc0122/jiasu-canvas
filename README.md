...
<h1 align="center">佳速画布 (jiasu-canvas)</h1>
...
佳速画布是一款面向图片创作的开源工作台。它把画布编排、AI 图片生成、参考图编辑、对话助手、提示词库和素材沉淀放在同一个界面里，适合用来探索视觉方案并连续迭代图片结果。
...
可打包成 Windows 安装包 / 绿色包，以及 Linux AppImage、deb 和 tar.gz。主进程用 `loadFile` 加载本地 `web/dist`，不依赖 localhost。完整步骤见 [desktop/README.md](desktop/README.md)。
